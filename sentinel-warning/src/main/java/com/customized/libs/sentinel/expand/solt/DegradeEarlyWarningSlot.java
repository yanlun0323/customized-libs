...
public class DegradeEarlyWarningSlot extends AbstractLinkedProcessorSlot<DefaultNode> {
    ...
    private List<DegradeRule> getRuleProvider(String resource) throws InvocationTargetException, IllegalAccessException {
    ...
    private DegradeRule getOriginRule(String resource) {
        List<DegradeRule> originRule = DegradeRuleManager.getRules()
                .stream().filter(rule -> rule.getResource().equals(resource)).collect(Collectors.toList());
        ...
        if (rules != null) {
            for (DegradeRule rule : rules) {
                if (!rule.passCheck(context, node, count)) {
                    ...
                    String msg = String.format("DegradeEarlyWarning:服务{%s}目前的熔断指标已经超过{%s}，接近配置的熔断阈值:{%s},"
                            , resource, rule.getCount(), originRuleCount);
                    log.info(msg);
                    alarmChain.sendAlarm(msg);
                    break;
                }
            }
        }
